...
for gidx in ['0'
    ...
    _dir = '/blue/narayanan/c.lovell/simba/m100n1024/run/snap_046/gal_%s/*.rtout.image'%_gal
    ...
    result = np.array([i for i in result if i is not None])
    ...
    for _orientation in np.arange(orientations): 
        ...
        np.savetxt('arrays/alma_%s.txt'%fname, im_array)
        ...
        fig.savefig('images/alma_%s.png'%fname, bbox_inches='tight',dpi=300)
        plt.close()
