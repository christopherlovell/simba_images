...
if [ "$job_flag" -eq 1 ]; then
    ...
    if [ $N -gt 1000 ]; then
        echo "#SBATCH -t 0-02:00" >>$qsubfile
    else
        echo "#SBATCH -t 0-02:00" >>$qsubfile
    fi

    echo "#SBATCH --ntasks=8">>$qsubfile
    echo "#SBATCH --cpus-per-task=1">>$qsubfile
    ...
    echo "module load gnu_comp/7.3.0">>$qsubfile
    echo "module load hdf5/1.10.3">>$qsubfile
    echo -e "\n">>$qsubfile
    
    echo "module load pythonconda3/4.5.4">>$qsubfile
    echo "source activate pday">>$qsubfile
    echo -e "\n">>$qsubfile
    
    echo "PD_FRONT_END=\"/cosma7/data/dp004/dc-love2/codes/powderday/pd_front_end.py\"">>$qsubfile
    echo -e "\n">>$qsubfile

    echo "if [ \$((\$SLURM_ARRAY_TASK_ID+1)) -lt \$((${N}-1000)) ]; then">>$qsubfile
    echo "    id=\$(head -n \$((\$SLURM_ARRAY_TASK_ID+1001)) ids.txt | tail -n 1)">>$qsubfile
