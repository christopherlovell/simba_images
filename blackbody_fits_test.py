...
def mbb_pl(x,z,T,beta,alpha,Nbb,lambda_0=100.e-6):
    k = 1.381e-23; c = 2.998e8; h = 6.626e-34
    ...
    #_c = 2.9e8 * 1e6
    Npl = (Nbb * mbb(lambda_c,z,T,beta,lambda_0))# / (lambda_c)**3 # alpha

    # _mbb = mbb(_x,z,F,T,beta)
    _mbb = mbb(_x,z,T,beta,lambda_0)

    # return (Nbb*_mbb) + (Npl * (_x)**alpha * np.exp(-1 * ((_x/lambda_c)**2)))
    return (Nbb*_mbb) + (Npl * (_x/lambda_c)**(-1*alpha) * np.exp(-1 * ((_x/lambda_c)**2)))
...
lambda_peak = wav[np.argmax(spec,axis=1)] # micron
Tpeak = 2.9e3 / lambda_peak # K
...
# popt, pcov = curve_fit(lambda x, T, _alpha, Nbb: \
#                        mbb_pl(x,z,T,_beta,_alpha,Nbb), 
#                        wl, _y, p0=[40,-1.5,41],maxfev=int(1e4))
...
Lsol = _out.to(u.solLum).value
print(popt)

_mask = (wav>1e1) & (wav<1e3)
_nu = (constants.c / (wav[_mask] * u.micron)).to(u.Hz)
_spec = (spec[:,_mask] * (u.erg / u.s)) / (4 * np.pi * dl**2) / _nu
...
# ax.plot(x, mbb_pl(x,z,popt[0],_beta,popt[1],popt[2]))
ax.plot(x, mbb_norm(x,z,popt[0],_beta,popt[1],_lambda_0))
...
for i,_g in enumerate(galaxies):
    ...
    # ax.scatter(np.log10(Lsol[gidx]), Tpeak[gidx],# yerr=np.std(Lsol[gidx]) ,
