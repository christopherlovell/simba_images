...
def recalculate_ang_mom_vector(_g):
    """
    ...
    with h5py.File(fname, 'r') as f:
        _masses = f['PartType4/Masses'][_g.slist_start:_g.slist_end] 
        _vels = f['PartType4/Velocities'][_g.slist_start:_g.slist_end] 
        _coods = f['PartType4/Coordinates'][_g.slist_start:_g.slist_end] 
         
    _vels -= _g.vel.value 
    _coods -= _g.pos.value 
     
    L = np.sum(np.cross(_coods, (_vels * _masses[:,None])),axis=0)
    return L 
...
# theta = np.array([0,0,0,0,90,270]) * (np.pi/180.)
# phi = np.array([180,270,0,90,0,0]) * (np.pi/180.)
# phi = np.array([0,0,0,0,90,90]) * (np.pi/180.)
# theta = np.array([0,90,180,270,90,270]) * (np.pi/180.)
...
for gidx in [3,8,51,54,94,100,134,139]:
    ...
    _L = recalculate_ang_mom_vector(_g)
