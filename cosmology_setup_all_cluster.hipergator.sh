...
if [ "$job_flag" -eq 1 ]; then
    ...
    echo "#SBATCH --qos narayanan" >>$qsubfile
    echo "#SBATCH --job-name=${model_run_name}.snap${snap}" >>$qsubfile
    echo "#SBATCH --output=out/pd.master.snap${snap}.%a.%N.%j" >>$qsubfile
    echo "#SBATCH --error=err/pd.master.snap${snap}.%a.%N.%j" >>$qsubfile
    ...
    echo "module load module load git/2.14.1 gcc/8.2.0 openmpi/4.0.1 hdf5/1.10.1">>$qsubfile
    echo -e "\n">>$qsubfile
    
    echo "source ~/env/bin/activate">>$qsubfile
    echo -e "\n">>$qsubfile
